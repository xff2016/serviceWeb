...
//Array ( [mycookie] => 孙必达 [UserName] => 孙必达 [PHPSESSID] => ajrn4mquba7t2euuor81fnov47 ) 回收专员 ：孙必达，登陆成功!
try {
    ...
    //$res=$userData->create(array("kind"=>$kind,"phone"=>$phone,"name"=>$name,"address"=>$address,"product"=>$product,"model"=>$model,"extratext"=>$extratext); //添加对象
    ...
    { 
     ...
     $kind = $array['kind'];
     $mobilePhoneNumber = $array['mobilePhoneNumber'];
     $email = $array['email'];
    }

    setcookie('UserName',urlencode($username),time()+24*3600,'/');
    setcookie('kind',urlencode($kind),time()+24*3600,'/');
    ...
    if($kind == "administrator"){
                ...
                header("refresh:0;url=../html/management.html");
            }
    else if($kind == "recycler"){

                header("refresh:0;url=../html/recycler.html");
    }
    else if($kind == "maintenance"){

                header("refresh:0;url=../html/maintenance.html");
    }
